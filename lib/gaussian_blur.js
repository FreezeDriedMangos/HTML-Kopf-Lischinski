...
* @param gray - flag to show RGB or Grayscale image
*/
function gauss_internal(pixels, kernel, ch, boundaries, blurEpicenters){ 
  var data = pixels.data;
  var w = pixels.width;
  var h = pixels.height;
  var buff = new Uint8Array(w*h); 
  ...
  // don't blur this pixel if there's no ghost spline boundary in this row on or between the columns [i-mk, i+mk]
  // and in the column processing loop, don't blur this pixel if there's no ghost spline boundary in this column on or between the rows [j-mk, j+mk]
  const blurEpicentersByRow = {}
  const blurEpicentersByCol = {}
  blurEpicenters.forEach(blurEpicenterIndex => {
    const row = Math.trunc(blurEpicenterIndex / w)
    const col = Math.trunc(blurEpicenterIndex % w)
    blurEpicentersByRow[row] = blurEpicentersByRow[row] || []
    blurEpicentersByRow[row].push(col)
    blurEpicentersByCol[col] = blurEpicentersByCol[col] || []
    blurEpicentersByCol[col].push(row)
  })
  ...
  for (var j = 0, hw = 0; j < h; j++, hw += w)  // j is the row, hw is the index of the first pixel in that row
  {
    for (var i = 0; i < w; i++) // i is the column
    {
      const isBlurLocation = blurEpicentersByRow[j]?.filter(epicenterColumn => i-mk <= epicenterColumn && epicenterColumn <= i+mk).length > 0
      if (!isBlurLocation) {
        buff[hw + i] = data[(hw + i)*4 + ch]
        continue
      }
  ...
  for (var j = 0; j < h; j++) // j is the row
  {
    for (var i = 0; i < w; i++) // i is the column
    {
      ...
      const isBlurLocation = blurEpicentersByCol[i]?.filter(epicenterRow => j-mk <= epicenterRow && epicenterRow <= j+mk).length > 0
      if (!isBlurLocation) {
  ...
  */
  function gauss(canvas, sigma, boundaries=[], blurEpicenters=[]){
    var context = canvas.getContext('2d');
    var pixels = context.getImageData(0, 0, canvas.width,canvas.height);
    var kernel = makeGaussKernel(sigma);
    ...
    // Blur a cahnnel (RGB or Grayscale)
    for (var ch = 0; ch <= 3; ch++){
      gauss_internal(pixels, kernel, ch, boundaries || [], blurEpicenters || []);
    }
    // Apply the modified pixels
    context.putImageData(pixels, 0, 0);
  }