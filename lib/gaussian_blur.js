...
function gauss_internal(pixels, kernel, ch, boundaries, blendDistance){ 
  ...
  for (var j = 0, hw = 0; j < h; j++, hw += w)  // j is the row, hw is the index of the first pixel in that row
  {
    for (var i = 0; i < w; i++) // i is the column
    {
      if (blendDistance && 0xff-blendDistance[(hw + i)*4 + 2] > kl+2) { // for convenience in calculating blendDistance, 255 means "i'm right on top of a blend boundary" and 0 means "I'm 255 pixels away from the nearest blend boundary"
        buff[hw + i] = data[(hw + i)*4 + ch]
        skippedRows++
        continue
      }
  ...
  for (var j = 0; j < h; j++) // j is the row
  {
    for (var i = 0; i < w; i++) // i is the column
    {
      ...
      if (blendDistance && 0xff-blendDistance[off + 2] > kl+2) { // for convenience in calculating blendDistance, 255 means "i'm right on top of a blend boundary" and 0 means "I'm 255 pixels away from the nearest blend boundary"
