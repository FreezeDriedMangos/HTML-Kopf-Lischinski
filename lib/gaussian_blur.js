...
function gauss_internal(pixels, kernel, ch, boundaries){ 
  ...
  for (var j = 0, offset = 0; j < h; j++, offset += w) // j is the row, offset is unused? I think it's a nonsense variable
  {
    for (var i = 0; i < w; i++) // i is the column
    {
      var sum = 0;
      ...
      for (k = 0; k < kl; k++)
      {
        // note: what's done above must be done here as well, only for rows instead of columns
      ...
      var off = (j*w + i)*4;
      data[off + ch] = sum / (1 - skippedKernelSum) // renormalize the kernel for the skipped parts
    }
  }
}
  ...
  */
  function gauss(canvas, sigma, boundaries=[]){
    var context = canvas.getContext('2d');
    var pixels = context.getImageData(0, 0, canvas.width,canvas.height);
    var kernel = makeGaussKernel(sigma);
    ...
    // Blur a cahnnel (RGB or Grayscale)
    for (var ch = 0; ch <= 3; ch++){
      gauss_internal(pixels, kernel, ch, boundaries || []);
    }
    // Apply the modified pixels
    context.putImageData(pixels, 0, 0);
  }