...
* @param gray - flag to show RGB or Grayscale image
*/
function gauss_internal(pixels, kernel, boundaries, blendDistance){ 
  var data = pixels.data;
  var w = pixels.width;
  var h = pixels.height;
  var buff = new Uint8Array(w*h*4); 
  var mk = Math.floor(kernel.length / 2);
  var kl = kernel.length;
  ...
  // new optimization idea: first, do a distance field floodfill from all the ghost splines (drawing in all splines as pure white first, and counting only ghost splines as seeds)
  // and pass the results in to this function
  // then, in each loop, if the pixel's greyscale value is 0 or > mk+2, skip it (bc it's too far away from any meeting of blendable colors to ever get blended (note: 0 would mean it never got filled bc there was no seed (aka ghost spline point) within the boundary that pixel is located in))
  //
  // this optimization is imperfect when pixelSize is greater than... 126?
  ...
  for (var j = 0, hw = 0; j < h; j++, hw += w)  // j is the row, hw is the index of the first pixel in that row
  {
    for (var i = 0; i < w; i++) // i is the column
    {
      if (blendDistance && 0xff-blendDistance[(hw + i)*4 + 2] > mk+2) { // for convenience in calculating blendDistance, 255 means "i'm right on top of a blend boundary" and 0 means "I'm 255 pixels away from the nearest blend boundary"
        for (var ch = 0; ch <= 3; ch++){
          buff[(hw + i)*4 + ch] = data[(hw + i)*4 + ch]
        }
        continue
      }
      ...
      for (var k = 0; k < kl; k++)
      {
        ...
        for (var ch = 0; ch <= 3; ch++){
          sum[ch] += data[(hw + col)*4 + ch]*kernel[k];
        }
      }

      for (var ch = 0; ch <= 3; ch++){
        buff[(hw + i)*4 + ch] = sum[ch] / (1 - skippedKernelSum) // renormalize the kernel for the skipped parts
      }
    }
  }
  ...
  for (var j = 0; j < h; j++) // j is the row
  {
    for (var i = 0; i < w; i++) // i is the column
    {
      ...
      if (blendDistance && 0xff-blendDistance[off + 2] > mk+2) { // for convenience in calculating blendDistance, 255 means "i'm right on top of a blend boundary" and 0 means "I'm 255 pixels away from the nearest blend boundary"
        for (var ch = 0; ch <= 3; ch++){
          data[off + ch] = buff[(j*w + i)*4 + ch]
        }
        continue
      }
      ...
      for (k = 0; k < kl; k++)
      {
        ...
        for (var ch = 0; ch <= 3; ch++){
          sum[ch] += buff[(row*w + i)*4 + ch]*kernel[k];
        }
      }

      if (skippedKernelSum < 1) 
        for (var ch = 0; ch <= 3; ch++)
          data[off + ch] = sum[ch] / (1 - skippedKernelSum) // renormalize the kernel for the skipped parts
    }
  }
  ...
  console.log('Finished blurring channel ' + ch)
}
  ...
  */
  function gauss(canvas, sigma, boundaries=[], blendDistance=undefined){
    ...
    gauss_internal(pixels, kernel, boundaries || [], blendDistance);
