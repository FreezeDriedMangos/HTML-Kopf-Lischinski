...
class BSpline(object):
    ...
    def __call__(self, u):
        ...
        for tup in enumerate(self.KnotVector):
            (k, uk) = tup
            if uk >= u:
                break
        if s == 0:
    ...
    def Quadratic_Bezier_Fit(self):
        ...
        for tup in zip(control_points, on_curve_points[1:]):
            (cp, ocp1) = tup
    ...
    def Integrate(self, index, func, intervals):
        ...
        s = 0
        for span in spans:
            s += self.Integrate_part(func, span, intervals)
        return s
    ...
    def reversed(self):
        arr = []
        for k in reversed(self.KnotVector):
            arr.append(1-k)
...
class Closed_BSpline(BSpline):
    ...
    def __init__(self, KnotVector, points, degree=None):
        BSpline.__init__(KnotVector, points, degree)
    ...
    def Move(self, index, value):
        ...
        BSpline.Move(index, value)
        if index < self.degree:
            BSpline.Move(index + self._unwrapped_len, value)
